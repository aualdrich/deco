...
RSpec.describe Card, type: :model do
  ...
  describe "associations" do
    it "belongs to a project" do
      card = described_class.new(title: "My Card", project: project)
  ...
  describe "validations" do
    it "is valid with a title and project" do
      card = described_class.new(title: "Valid Card", project: project)
      expect(card).to be_valid
    end

    it "is invalid without a title" do
      card = described_class.new(title: nil, project: project)
      expect(card).not_to be_valid
      expect(card.errors[:title]).to include("can't be blank")
    end

    it "is invalid without a project" do
      card = described_class.new(title: "Orphan Card")
      expect(card).not_to be_valid
    end

    it "is invalid with a status outside of STATUSES" do
      card = described_class.new(title: "Bad Status", project: project, status: "not-a-real-status")
      expect(card).not_to be_valid
  ...
  describe "status normalization" do
    it "normalizes kebab-case statuses to snake_case" do
      card = project.cards.create!(title: "Normalize", status: "ready-to-implement")
      expect(card.status).to eq("ready_to_implement")

      card.update!(status: "in-review")
      expect(card.status).to eq("in_review")
    end
  end

  describe "chat_messages" do
    it "defaults to an empty array" do
      card = project.cards.create!(title: "Default Chat", status: "todo")
      expect(card.chat_messages).to eq([])
    end

    it "serializes and deserializes an array of hashes" do
      messages = [
        { "role" => "user", "content" => "Hello", "timestamp" => Time.current.iso8601 },
        { "role" => "assistant", "content" => "Hi there", "timestamp" => Time.current.iso8601 }
      ]
      ...
      card.reload
      expect(card.chat_messages).to eq(messages)
      expect(card.chat_messages.first).to be_a(Hash)
  ...
  describe "scopes" do
    ...
    describe ".active" do
      it "returns only non-archived cards" do
        expect(described_class.active).to include(active_card)
        expect(described_class.active).not_to include(archived_card)
      end
    end

    describe ".archived" do
      it "returns only archived cards" do
        expect(described_class.archived).to include(archived_card)
        expect(described_class.archived).not_to include(active_card)
      end
    end
  end
